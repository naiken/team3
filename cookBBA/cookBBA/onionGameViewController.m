...
#import "onionGameViewController.h"

@interface onionGameViewController ()

@end

@implementation onionGameViewController {
...
- (void)viewDidLoad
{
    ...
    //タイマーの初期設定
    animationTimer = [NSTimer scheduledTimerWithTimeInterval:0.02 target:self selector:@selector(pancoAnimation:) userInfo:nil repeats:YES];
...
- (void)pancoAnimation:(NSTimer*)timer {
    ...
    switch (scorePoint) {
        case 2:
        {
            UIImage* pancoImg = [UIImage imageNamed:@"tamanegi05.png"];
            _onionCup.image = pancoImg;
        }
            break;
            
        case 7:
        {
            UIImage* pancoImg = [UIImage imageNamed:@"tamanegi04.png"];
            _onionCup.image = pancoImg;
        }
            break;
        ...
        case 12:
        {
            UIImage* pancoImg = [UIImage imageNamed:@"tamanegi03.png"];
            _onionCup.image = pancoImg;
        }
            break;
        ...
        case 17:
        {
            UIImage* pancoImg = [UIImage imageNamed:@"tamanegi02.png"];
            _onionCup.image = pancoImg;
        }
            break;
        ...
        case 22:
        {
            UIImage* pancoImg = [UIImage imageNamed:@"tamanegi01.png"];
            _onionCup.image = pancoImg;
        }
...
- (void)badResult:(int)tag {
    ...
    if (tag == 1) {
        UIImage* badOnion = [UIImage imageNamed:@"tamanegi01.png"];
        _onionCup.image = badOnion;
    }
...
- (void)greatResult {
    ...
    //リザルトgreatの描画
    UIImage* greatImg = [UIImage imageNamed:@"tamanegi_great.png"];
    _onionCup.image = greatImg;
...
- (void)segueToPancoGame {
    [self performSegueWithIdentifier:@"hamburgGameSegue" sender:self];
}

- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.identifier isEqualToString:@"hamburgGameSegue"]) {
